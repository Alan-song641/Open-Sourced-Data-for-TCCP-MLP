...
def main():
    ...
    if config.config['fp_engine'] == 'Fortran':
        ...
        nl.calculate(trainingImages, fortran=False) #ZB
        et = time.time()
        ...
        #acfs, fpRange = represent_BP(nl, nFPs, trainingimages, properties, #ZB
        #                           G_paras=fp_paras, fpfilename=srcData, #ZB
        #                           fortran=True, logger=logger) #ZB
        logger.info('Calculating fingerprints') ###ORIGINAL #ZB
        #acfs, fpRange = represent_BP(nl, nFPs, trainingimages, properties, #ZB
        acfs, fpRange, magnitudeScale, interceptScale = represent_BP(nl, nFPs, trainingImages, properties, #ZB 
                                    G_paras=fp_paras, fpfilename=srcData, #ZB
                                    fortran=False, logger=logger) #ZB
    ...
    if config.config['init_model_parameters']:
        ...
        # then uncomment the "FOR BENCHMARKING" in training.py 
        # and set ifelectronegativity = False, init_model_parameters = True
        ...
        params = list()
        # Ti: input 2 G1
        params.append(torch.tensor([[-0.0197, -0.2215],
                                    [-0.1512,  0.1380]], requires_grad=True))
        params.append(torch.tensor([0.1985, 0.1296], requires_grad=True))
        # first NN hidden layer
        params.append(torch.tensor([[ 0.0288, -0.0234],
                                    [ 0.1070,  0.0561]], requires_grad=True))
        params.append(torch.tensor([ 0.0125, -0.0228], requires_grad=True))
        # second NN hidden layer
        params.append(torch.tensor([[-0.0537,  0.0714],
                                    [-0.0167, -0.0806]], requires_grad=True))
        params.append(torch.tensor([ 0.0125, -0.0233], requires_grad=True))

        # O: input 2 G1
        params.append(torch.tensor([[-0.0147, -0.0115],
                                    [-0.1512,  0.1380]], requires_grad=True))
        params.append(torch.tensor([0.1985, 0.1296], requires_grad=True))
        # first NN hidden layer
        params.append(torch.tensor([[-0.0719,  0.0416],
                                    [-0.0316, -0.0806]], requires_grad=True))
        params.append(torch.tensor([-0.0371, -0.0196], requires_grad=True))
        # second NN hidden layer
        params.append(torch.tensor([[-0.0493,  0.0295],
                                        [-0.1631,  0.0273]], requires_grad=True))
        params.append(torch.tensor([0.0086, 0.0160], requires_grad=True))
    ...
    if config.config['model_type'] == 'neural_network' and config.config['restart'] == False:
        ...
        model = NeuralNetwork(
                    ...
                    scaler=scaler,
                    debug=config.config['nn_values'],
                    ifElectronegativity=False, # restart = True
                    if_short=config.config['if_short'],
                    if_long=config.config['if_long'],
                    process_number=nProc,
                    partitions=partitions
