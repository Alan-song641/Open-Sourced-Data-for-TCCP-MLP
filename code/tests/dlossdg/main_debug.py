...
def main():
    ...
    print('PLEASE call this function under dlossdg folder!!!')
    
    def one_step(params, allElement_fps, elem):
        ...
        for i, data in enumerate(zip(batches, batches_Ele)):
            ...
            predEnergies, predForces,\
                charges_dict, charge_percent = model(
                                        allElement_fps, batch.dgdx, # NOTE: here the input fps is turbulated
                                        batch, batch_ele,
                                        device, logger=logger)
    ...
    len_param = 3 # 32 in [2*32]
    tot_len = 2 # one G1 and one G2 ,so it is 2 in [2*32]
    elem = 'Ti'
    ...
    for which_layer in range(tot_len): 
        for which_param in range(len_param):
            ...
            for iter in [1, -1, 0]: # left turbulence, right turbulence, original to get analytical
                ...
                # build the allElement_fps from scratch for each loop
                partitions = DataPartitioner(srcData=srcData,
                        fpRange=fpRange,
                        magnitudeScale=magnitudeScale,
                        interceptScale=interceptScale,
                ...
                batches = torch.utils.data.DataLoader(partition, batch_size=batch_size, collate_fn=batchGenerator, shuffle=False, **kwargs)
                for batch in batches:
                    allElement_fps = batch.allElement_fps # [2*32] shape

                allElement_fps[elem][which_param, which_layer].data += iter * step_size 
                loss, lossgrads, loss_e, lossgrads_e, loss_f, lossgrads_f, lossgrads_q, charge_dict = one_step(params, allElement_fps, elem)
                
                numerical_loss.append(loss.item())
                numerical_loss_e.append(loss_e.item())
                numerical_loss_f.append(loss_f.item())
                numerical_loss_q.append(torch.sum(charge_dict[elem]))
            ...
            tot_ana_l.append(lossgrads[which_param, which_layer].item())

            tot_num_e.append(num_e)
            tot_ana_e.append(lossgrads_e[which_param, which_layer].item())

            tot_num_f.append(num_f)
            tot_ana_f.append(lossgrads_f[which_param, which_layer].item())

            tot_num_q.append(num_q)
            tot_ana_q.append(lossgrads_q[which_param, which_layer].item())

            print(f'which_layer: {which_layer}, which_param: {which_param}')


    # NOTE: unwrap the array 
    print('[Energy]:')
    print('         numerical       analytical      difference')
    for i in range(len(tot_ana_e)):
        print('Energy: {:.8f}      {:.8f}      {:.8f}'.format(tot_num_e[i], tot_ana_e[i],  np.divide(tot_ana_e[i], tot_num_e[i])*100 ))
    print()

    print('[Forces]:')
    print('         numerical       analytical      difference')
    for i in range(len(tot_num_f)):
        print('Forces: {:.8f}      {:.8f}      {:.2f}'.format(tot_num_f[i], tot_ana_f[i], np.divide(tot_ana_f[i], tot_num_f[i])*100 ))
    print()

    print('[q-hat]:')
    print('         numerical       analytical      difference')
    for i in range(len(tot_num_q)):
        print('q-hat: {:.8f}      {:.8f}      {:.2f}'.format(tot_num_q[i].item(), tot_ana_q[i], np.divide(tot_ana_q[i], tot_num_q[i].item())*100 ))   
    print()

    os.system('rm batches/ fingerprints/ *.log -rf')

    '''
        PLEASE call this function under dlossdg folder!!!
    which_layer: 0, which_param: 0
    which_layer: 0, which_param: 1
    which_layer: 0, which_param: 2
    which_layer: 1, which_param: 0
    which_layer: 1, which_param: 1
    which_layer: 1, which_param: 2
    [Energy]:
            numerical       analytical      difference
    Energy: -0.00011825      -0.00011825      99.99999973
    Energy: -0.00011084      -0.00011084      99.99999949
    Energy: -0.00010911      -0.00010911      100.00000037
    Energy: -0.00000404      -0.00000404      100.00000224
    Energy: -0.00000588      -0.00000588      99.99999001
    Energy: -0.00000556      -0.00000556      100.00001197

    [Forces]:
            numerical       analytical      difference
    Forces: 0.00074850      0.00074850      100.00
    Forces: -0.00066799      -0.00066799      100.00
    Forces: -0.00140127      -0.00140127      100.00
    Forces: 0.00006571      0.00006571      100.00
    Forces: 0.00000769      0.00000769      100.00
    Forces: -0.00003787      -0.00003787      100.00

    [q-hat]:
            numerical       analytical      difference
    q-hat: 0.00735008      0.00735008      100.00
    q-hat: 0.00815753      0.00815753      100.00
    q-hat: 0.00805300      0.00805300      100.00
    q-hat: 0.00024915      0.00024915      100.00
    q-hat: 0.00043046      0.00043046      100.00
    q-hat: 0.00040755      0.00040755      100.00
